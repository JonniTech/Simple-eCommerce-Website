                        ...
                        <div key={user._id} className="bg-white dark:bg-[#362F5C] p-6 rounded-3xl shadow-sm border border-transparent dark:border-white/5 flex flex-col justify-between group hover:border-primary/20 transition-all">
                            <div className="flex items-start justify-between overflow-x-auto pb-2">
                                <div className="flex items-center gap-4 min-w-max">
                                    <div className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl shadow-inner flex-shrink-0 ${user.isAdmin ? "bg-primary text-white" : "bg-gray-100 dark:bg-white/10 text-gray-500"
                                        }`}>
                                        {user.isAdmin ? <FiShield /> : <FiUser />}
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-lg text-dark dark:text-white whitespace-nowrap">{user.name}</h3>
                                        <div className="flex items-center gap-2 text-sm text-gray-500 mt-1 whitespace-nowrap">
                                ...
                                {user.isAdmin && (
                                    <span className="bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ml-4 flex-shrink-0">
                                        Admin
                                    </span>
                                )}
                            </div>
